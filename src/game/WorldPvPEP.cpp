        ...
        case GO_BATTLEFIELD_BANNER_PLAGUELANDS_1:
            switch(uiEventId)
            {
                case EVENT_NORTHPASS_WIN_ALLIANCE:
                    ProcessCaptureEvent(WIN_A, pPlayer->GetTeam(), TOWER_ID_NORTHPASS);
                    break;
                case EVENT_NORTHPASS_WIN_HORDE:
                    ProcessCaptureEvent(WIN_H, pPlayer->GetTeam(), TOWER_ID_NORTHPASS);
                    break;
                case EVENT_NORTHPASS_CONTEST_ALLIANCE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_A, pPlayer->GetTeam(), TOWER_ID_NORTHPASS);
                    break;
                case EVENT_NORTHPASS_CONTEST_HORDE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_H, pPlayer->GetTeam(), TOWER_ID_NORTHPASS);
                    break;
                case EVENT_NORTHPASS_PROGRESS_ALLIANCE:
                    ProcessCaptureEvent(PROGRESS_A, pPlayer->GetTeam(), TOWER_ID_NORTHPASS);
                    sWorld.SendZoneText(ZONE_ID_EASTERN_PLAGUELANDS, sObjectMgr.GetMangosStringForDBCLocale(LANG_OPVP_EP_CAPTURE_NPT_A));
                    break;
                case EVENT_NORTHPASS_PROGRESS_HORDE:
                    ProcessCaptureEvent(PROGRESS_H, pPlayer->GetTeam(), TOWER_ID_NORTHPASS);
        ...
        case GO_BATTLEFIELD_BANNER_PLAGUELANDS_2:
            switch(uiEventId)
            {
                case EVENT_CROWNGUARD_WIN_ALLIANCE:
                    ProcessCaptureEvent(WIN_A, pPlayer->GetTeam(), TOWER_ID_CROWNGUARD);
                    break;
                case EVENT_CROWNGUARD_WIN_HORDE:
                    ProcessCaptureEvent(WIN_H, pPlayer->GetTeam(), TOWER_ID_CROWNGUARD);
                    break;
                case EVENT_CROWNGUARD_CONTEST_ALLIANCE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_A, pPlayer->GetTeam(), TOWER_ID_CROWNGUARD);
                    break;
                case EVENT_CROWNGUARD_CONTEST_HORDE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_H, pPlayer->GetTeam(), TOWER_ID_CROWNGUARD);
                    break;
                case EVENT_CROWNGUARD_PROGRESS_ALLIANCE:
                    ProcessCaptureEvent(PROGRESS_A, pPlayer->GetTeam(), TOWER_ID_CROWNGUARD);
                    sWorld.SendZoneText(ZONE_ID_EASTERN_PLAGUELANDS, sObjectMgr.GetMangosStringForDBCLocale(LANG_OPVP_EP_CAPTURE_CGT_A));
                    break;
                case EVENT_CROWNGUARD_PROGRESS_HORDE:
                    ProcessCaptureEvent(PROGRESS_H, pPlayer->GetTeam(), TOWER_ID_CROWNGUARD);
        ...
        case GO_BATTLEFIELD_BANNER_PLAGUELANDS_3:
            switch(uiEventId)
            {
                case EVENT_EASTWALL_WIN_ALLIANCE:
                    ProcessCaptureEvent(WIN_A, pPlayer->GetTeam(), TOWER_ID_EASTWALL);
                    break;
                case EVENT_EASTWALL_WIN_HORDE:
                    ProcessCaptureEvent(WIN_H, pPlayer->GetTeam(), TOWER_ID_EASTWALL);
                    break;
                case EVENT_EASTWALL_CONTEST_ALLIANCE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_A, pPlayer->GetTeam(), TOWER_ID_EASTWALL);
                    break;
                case EVENT_EASTWALL_CONTEST_HORDE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_H, pPlayer->GetTeam(), TOWER_ID_EASTWALL);
                    break;
                case EVENT_EASTWALL_PROGRESS_ALLIANCE:
                    ProcessCaptureEvent(PROGRESS_A, pPlayer->GetTeam(), TOWER_ID_EASTWALL);
                    sWorld.SendZoneText(ZONE_ID_EASTERN_PLAGUELANDS, sObjectMgr.GetMangosStringForDBCLocale(LANG_OPVP_EP_CAPTURE_EWT_A));
                    break;
                case EVENT_EASTWALL_PROGRESS_HORDE:
                    ProcessCaptureEvent(PROGRESS_H, pPlayer->GetTeam(), TOWER_ID_EASTWALL);
        ...
        case GO_BATTLEFIELD_BANNER_PLAGUELANDS_4:
            switch(uiEventId)
            {
                case EVENT_PLAGUEWOOD_WIN_ALLIANCE:
                    ProcessCaptureEvent(WIN_A, pPlayer->GetTeam(), TOWER_ID_PLAGUEWOOD);
                    break;
                case EVENT_PLAGUEWOOD_WIN_HORDE:
                    ProcessCaptureEvent(WIN_H, pPlayer->GetTeam(), TOWER_ID_PLAGUEWOOD);
                    break;
                case EVENT_PLAGUEWOOD_CONTEST_ALLIANCE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_A, pPlayer->GetTeam(), TOWER_ID_PLAGUEWOOD);
                    break;
                case EVENT_PLAGUEWOOD_CONTEST_HORDE:
                    // handle event - in this case player's team will be the opposite team
                    ProcessCaptureEvent(CONTESTED_H, pPlayer->GetTeam(), TOWER_ID_PLAGUEWOOD);
                    break;
                case EVENT_PLAGUEWOOD_PROGRESS_ALLIANCE:
                    ProcessCaptureEvent(PROGRESS_A, pPlayer->GetTeam(), TOWER_ID_PLAGUEWOOD);
                    sWorld.SendZoneText(ZONE_ID_EASTERN_PLAGUELANDS, sObjectMgr.GetMangosStringForDBCLocale(LANG_OPVP_EP_CAPTURE_PWT_A));
                    break;
                case EVENT_PLAGUEWOOD_PROGRESS_HORDE:
                    ProcessCaptureEvent(PROGRESS_H, pPlayer->GetTeam(), TOWER_ID_PLAGUEWOOD);
    ...
    switch(uiType)
    {
        case TYPE_CROWNGUARD_CONTROLLER:
            if (uiData == ALLIANCE)
            {
                ...
                else if (GetData(TYPE_CROWNGUARD_STATE) == PROGRESS_A)
                {
                ...
                else if (GetData(TYPE_CROWNGUARD_STATE) == CONTESTED_A)
                    m_uiCrownguardWorldState = WORLD_STATE_CROWNGUARD_CONT_HORDE;
                else if (GetData(TYPE_CROWNGUARD_STATE) == WIN_A)
            ...
            else if (uiData == HORDE)
            {
                ...
                if (GetData(TYPE_CROWNGUARD_STATE) == PROGRESS_H)
                {
        ...
        case TYPE_EASTWALL_CONTROLLER:
            if (uiData == ALLIANCE)
            {
                ...
                else if (GetData(TYPE_EASTWALL_STATE) == PROGRESS_A)
                {
                ...
                // contested states are also sent by the opposite team
                // alliance contests horde tower
                else if (GetData(TYPE_EASTWALL_STATE) == CONTESTED_A)
                    m_uiEastwallWorldState = WORLD_STATE_EASTWALL_CONT_HORDE;
                else if (GetData(TYPE_EASTWALL_STATE) == WIN_A)
            ...
            else if (uiData == HORDE)
            {
                ...
                if (GetData(TYPE_EASTWALL_STATE) == PROGRESS_H)
                {
                ...
                // contested states are also sent by the opposite team
                // horde contests alliance tower
                else if (GetData(TYPE_EASTWALL_STATE) == CONTESTED_H)
                    m_uiEastwallWorldState = WORLD_STATE_EASTWALL_CONT_ALY;
                else if (GetData(TYPE_EASTWALL_STATE) == WIN_H)
                    m_uiEastwallWorldState = WORLD_STATE_EASTWALL_HORDE;
            }
            // set controller only for progress and neutral
            if (GetData(TYPE_EASTWALL_STATE) == PROGRESS_A)
        ...
        case TYPE_NORTHPASS_CONTROLLER:
            if (uiData == ALLIANCE)
            {
                ...
                else if (GetData(TYPE_NORTHPASS_STATE) == PROGRESS_A)
                {
                ...
                // contested states are also sent by the opposite team
                // alliance contests horde tower
                else if (GetData(TYPE_NORTHPASS_STATE) == CONTESTED_A)
                    m_uiNorthpassWorldState = WORLD_STATE_NORTHPASS_CONT_HORDE;
                else if (GetData(TYPE_NORTHPASS_STATE) == WIN_A)
            ...
            else if (uiData == HORDE)
            {
                ...
                if (GetData(TYPE_NORTHPASS_STATE) == PROGRESS_H)
                {
                ...
                // contested states are also sent by the opposite team
                // horde contests alliance tower
                else if (GetData(TYPE_NORTHPASS_STATE) == CONTESTED_H)
                    m_uiNorthpassWorldState = WORLD_STATE_NORTHPASS_CONT_ALY;
                else if (GetData(TYPE_NORTHPASS_STATE) == WIN_H)
                    m_uiNorthpassWorldState = WORLD_STATE_NORTHPASS_HORDE;
            }
            // set controller only for progress and neutral
            if (GetData(TYPE_NORTHPASS_STATE) == PROGRESS_A)
        ...
        case TYPE_PLAGUEWOOD_CONTROLLER:
            if (uiData == ALLIANCE)
            {
                ...
                else if (GetData(TYPE_PLAGUEWOOD_STATE) == PROGRESS_A)
                {
                ...
                // contested states are also sent by the opposite team
                // alliance contests horde tower
                else if (GetData(TYPE_PLAGUEWOOD_STATE) == CONTESTED_A)
                    m_uiPlaguewoodWorldState = WORLD_STATE_PLAGUEWOOD_CONT_HORDE;
                else if (GetData(TYPE_PLAGUEWOOD_STATE) == WIN_A)
            ...
            else if (uiData == HORDE)
            {
                ...
                if (GetData(TYPE_PLAGUEWOOD_STATE) == PROGRESS_H)
                {
                ...
                // contested states are also sent by the opposite team
                // horde contests alliance tower
                else if (GetData(TYPE_PLAGUEWOOD_STATE) == CONTESTED_H)
                    m_uiPlaguewoodWorldState = WORLD_STATE_PLAGUEWOOD_CONT_ALY;
                else if (GetData(TYPE_PLAGUEWOOD_STATE) == WIN_H)
                    m_uiPlaguewoodWorldState = WORLD_STATE_PLAGUEWOOD_HORDE;
            }
            // set controller only for progress and neutral
            if (GetData(TYPE_PLAGUEWOOD_STATE) == PROGRESS_A)
