...
{
    WorldPacket data(SMSG_SPELLNONMELEEDAMAGELOG, (9+9+4+4+1+4+4+1+1+4+4+1)); // we guess size
...
void Unit::ShowMovementInfo(const uint32 pl, MovementInfo const& movementInfo, const char *opcode)
{
	if (sWorld.TargetGuid == pl)
	{
		...
		sLog.outBasic(" tm:%d ftm:%d | tG: %d", movementInfo.GetTime(), movementInfo.GetFallTime(), movementInfo.GetTransportGuid().GetEntry());
		sLog.outBasic("");
	}
}
