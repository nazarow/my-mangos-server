...
void GameObject::Update(uint32 update_diff, uint32 diff)
{
    ...
    if (GetGoType() == GAMEOBJECT_TYPE_CAPTURE_POINT)
    {
        if (m_captureTime < diff)
        {
            ...
            float radius = info->capturePoint.radius>>1;
            ...
            for (PlayersSet::iterator itr = m_CapturePlayersSet.begin(); itr != m_CapturePlayersSet.end(); ++itr)
            {
                // check use conditions:
                if (!(*itr)->CanCaptureTowerPoint())
                    return;
...
void GameObject::Use(Unit* user)
{
    ...
    switch(GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_CAPTURE_POINT:                 // 29
        {
            ...
            if (!user || (user->GetTypeId() != TYPEID_PLAYER))
                return;
            ...
            double m_sliderTick = double(m_RangePlayers) / double(info->capturePoint.maxSuperiority);

            double m_sliderWinner = 50.0f + (m_progressFaction == ALLIANCE)?(double(info->capturePoint.neutralPercent)/2):(-double(info->capturePoint.neutralPercent)/2);
            ...
            if (player->GetTeam() == m_progressFaction)
            {
                if (m_progressFaction == ALLIANCE)
                {
                    ...
                    else m_captureTicks = CAPTURE_SLIDER_ALLIANCE;
                }
                else if (m_progressFaction == HORDE)
                {
                    if (m_captureTicks > CAPTURE_SLIDER_HORDE)
                        m_captureTicks -= m_sliderTick;
                    else m_captureTicks = CAPTURE_SLIDER_HORDE;
                }
            }
