            ...
            case ALLIANCE:
                {
                    ...
                    if(atEntry)
                    {
                        ...
                        if(plr->GetDistance(atEntry->x,atEntry->y,atEntry->z) > 5.0f + atEntry->radius)
                        {
                            ...
                            if(!map)
                                return true;
							if(!go->Create(map->GenerateLocalLowGuid(HIGHGUID_GAMEOBJECT), SI_SILITHYST_MOUND, map, plr->GetPositionX(),plr->GetPositionY(),plr->GetPositionZ(),plr->GetOrientation(),0,0,0,0,100, GO_STATE_READY))
                            {
                                delete go;
                            }
                            else
