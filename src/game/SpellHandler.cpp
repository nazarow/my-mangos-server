...
{
	if (!GetPlayer() || !GetPlayer()->IsInWorld()) //kia
		return;
    ...
    {
		sLog.outMy("WORLD: got cast spell packet, spellId - %u, cast_count: %u data length = %i, pl:%s, m:%s",
			spellId, cast_count, (uint32)recvPacket.size(), GetPlayer()->GetName(), mover?mover->GetName():"--");
