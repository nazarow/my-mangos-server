...
void OutdoorPvPObjectiveNA::FactionTakeOver(Team team)
{
...
OutdoorPvPObjective(pvp), m_capturable(true), m_GuardsAlive(0), m_ControllingFaction(TEAM_NONE),
...
{
    AddCapturePoint(182210,530,-1572.57f,7945.3f,-22.475f,2.05949f,0,0,0.857167f,0.515038f);
}
...
void OutdoorPvPObjectiveNA::FillInitialWorldStates(WorldPacket &data, uint32 &count)
{
    if(m_ControllingFaction == ALLIANCE)
    {
...
bool OutdoorPvPObjectiveNA::HandleCustomSpell(Player * plr, uint32 spellId, GameObject * go)
{
    ...
    if(retval)
    {
        ...
        if(count > 0 && item)
        {
            ...
            sLog.outItems("Storage::CustomSpell [%u] %u,%u for %s", spellId, itemid, count, plr->GetName());
        }
