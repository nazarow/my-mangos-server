...
bool ChatHandler::HandleGameObjectStateCommand(char* args)
{
    ...
    uint32 stat;
    if (!ExtractOptUInt32(&args, stat, 0))
        return false;

	obj->SetGoState(GOState(stat));

    return true;
}

//change state selected object
bool ChatHandler::HandleGameObjectArtkitCommand(char* args)
{
    // number or [name] Shift-click form |color|Hgameobject:go_id|h[name]|h|r
    uint32 lowguid;
    if (!ExtractUint32KeyFromLink(&args, "Hgameobject", lowguid))
    ...
    uint32 art;
    if (!ExtractOptUInt32(&args, art, 0))
        return false;
