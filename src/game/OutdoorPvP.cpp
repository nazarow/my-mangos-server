...
bool OutdoorPvPObjective::AddCreature(uint32 type, uint32 entry, Team teamval, uint32 map, float x, float y, float z, float o, uint32 spawntimedelay)
{
...
bool OutdoorPvPObjective::AddCapturePoint(uint32 entry, uint32 map, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3)
{
    ...
    cdata.mapid = map;
    cdata.modelid_override = displayId;
...
bool OutdoorPvPObjective::Update(uint32 diff)
{
    ...
    uint32 Challenger = 0;
        ...
        if (sOutdoorPvPMgr.isOutdoorPvPTesting())
			m_ShiftPhase += (fact_diff>0)?float(m_ShiftMaxPhase/10.0f):float(-m_ShiftMaxPhase/10.0f);
		else
...
bool OutdoorPvPObjective::CanTalkTo(Player * plr, Creature * c, GossipMenu &gso)
{
    return false;
}
