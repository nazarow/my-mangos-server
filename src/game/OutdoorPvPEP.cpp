...
void OutdoorPvPObjectiveEP_EWT::FillInitialWorldStates(WorldPacket &data, uint32 &count)
{
...
void OutdoorPvPObjectiveEP_EWT::SummonSupportUnitAtNorthpassTower(Team team)
{
...
: OutdoorPvPObjective(pvp), m_TowerState(EP_TS_N), m_SummonedGOSide(TEAM_NONE)
{
...
void OutdoorPvPObjectiveEP_NPT::FillInitialWorldStates(WorldPacket &data, uint32 &count)
{
...
void OutdoorPvPObjectiveEP_NPT::SummonGO(Team team)
{
...
: OutdoorPvPObjective(pvp), m_TowerState(EP_TS_N), m_GraveyardSide(TEAM_NONE)
{
...
void OutdoorPvPObjectiveEP_CGT::FillInitialWorldStates(WorldPacket &data, uint32 &count)
{
...
void OutdoorPvPObjectiveEP_CGT::LinkGraveYard(Team team)
{
...
void OutdoorPvPObjectiveEP_PWT::FillInitialWorldStates(WorldPacket &data, uint32 &count)
{
...
void OutdoorPvPObjectiveEP_PWT::SummonFlightMaster(Team team)
{
    if(m_FlightMasterSpawned != team)
    {
        ...
        /*Creature * c = ObjectAccessor::GetCreatureInWorld(ObjectGuid(m_Creatures[EP_PWT_FLIGHTMASTER]));
        if(c)
        {
			c->isGossip
            GossipOption gso;
...
bool OutdoorPvPObjectiveEP_PWT::HandleGossipOption(Player *plr, uint64 guid, uint32 gossipid)
{
    ...
    if(itr != m_CreatureTypes.end())
    {
        ...
        if(itr->second == EP_PWT_FLIGHTMASTER)
        {
            ...
            plr->ActivateTaxiPathTo(nodes, cr, 0);
...
void OutdoorPvPEP::FillInitialWorldStates(WorldPacket & data, uint32 &count)
{
    ...
    {
        (*itr)->FillInitialWorldStates(data, count);
    }
}
